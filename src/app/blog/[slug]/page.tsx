...
export default async function Blog({ params: { slug } }: Readonly<Props>) {
  const { Blog } = (await PreprSdk.Blog({ slug })) || {};
  const allElements: React.ReactNode[] = [];

  Blog?.content?.forEach((item: any) => {
    const { _id, format, text, items } = item;
    ...
    allElements.push(
      <BlogElement tagname={format} key={_id}>
        {text}
      </BlogElement>,
    );
  });
...
function BlogElement({ tagname, children, ...props }: Readonly<BlogElementProps>) {
  let Tag = ((!tagname ? 'p' : htmlTags[tagname]?.name) || 'p') as keyof JSX.IntrinsicElements;
...
const htmlTags: Record<string, HtmlTag> = {
